...
Public Class Cart
    ...
    Private current_item As DataRow

    Public Sub New(ByRef parent As Control, i As String, source As DataRow)
        If source Is Nothing Then Return

        If source.Table.Columns.Contains("order_id") Then
            index = $"order_{i}_" & parent.Controls.Count
            id = source("order_id")
        ElseIf source.Table.Columns.Contains("cart_id") Then
            index = $"cart_{i}_" & parent.Controls.Count
            id = source("cart_id")
        End If
    ...
    Public Sub SetProperties()
        ...
        item_name.Text = current_item("item_name")
        price.Text = current_item("price")
        quantity.Text = current_item("quantity")

        Try
            Using bmp As Bitmap = Bitmap.FromFile(current_item("image_dir").ToString)
    ...
    Public Sub SetOrderProperties()
        item_name.Text = $"ORDER ID: {id}      Item: {current_item("item_name")} x{current_item("quantity")}"
        price.Text = $"                                    Price: {current_item("price")}    Cost: {current_item("cost")}    "
    ...
    Private Sub btnSelect_Clicked(sender As Object, e As EventArgs)
        ...
        Dim item As Dictionary(Of String, Object) = DataRowToDictionary(current_item)
        item("id") = id
        ...
        Select Case btnSelect.ForeColor
            ...
            Case Color.Black
                ...
                MakeItemSummary(CartPage.order_summary)
                SetCost(1)

                MakeTransactions(item, "insert_product")
    ...
    Public Sub MakeItemSummary(parent As Control)
        ...
        Me.lb_item_id.Text = "Item ID: " & current_item("product_id")
        Me.lb_quantity.Text = "Quantity: " & quantity.Text
        Me.lb_cost.Text = "Cost: ₱" & Val(current_item("price") * quantity.Text)
    ...
    Public Function CartItem() As Panel
        ...
        btnSelect.BackgroundImage = Global.ProjectWithCrystalReport2.My.Resources.Resources.icons8_unchecked_checkbox_30
        addQuant.BackgroundImage = Global.ProjectWithCrystalReport2.My.Resources.Resources.icons8_add_new_96
        subQuant.BackgroundImage = Global.ProjectWithCrystalReport2.My.Resources.Resources.icons8_indeterminate_checkbox_96
        itemImage.Image = Global.ProjectWithCrystalReport2.My.Resources.Resources.icons8_no_image_30
        itemImage.SizeMode = PictureBoxSizeMode.Zoom
