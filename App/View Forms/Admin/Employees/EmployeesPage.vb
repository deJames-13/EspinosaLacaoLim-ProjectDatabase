Public Class EmployeesPage
    ...
    Private Sub btnSearch_Click(sender As Object, e As EventArgs) Handles btnSearch.Click
        ...
        last_name LIKE '%{txtSearch.Text}%' OR
        middle_name LIKE '%{txtSearch.Text}%' OR
        email LIKE '%{txtSearch.Text}%' OR
        phone_number LIKE '%{txtSearch.Text}%' OR
        address LIKE '%{txtSearch.Text}%' 
        "
        form_table = QueryToGrid($"SELECT * FROM user WHERE {condition}", empGrid)
        rowCount.Text = form_table.Rows.Count
        empGrid.Show()
    End Sub
End Class